...
static inline i32 rand_num(i32 from, i32 to) {
...
i32 main() {
    ...
    for (i32 i = 0; i < count; i++) {
        Pixel pixel = {
            rand_num(red_from, red_to),
            rand_num(green_from, green_to),
            rand_num(blue_from, blue_to),
            rand_num(alpha_from, alpha_to)
        };
        circles_img->circle(circles_img, rand_num(0, SIZE), rand_num(0, SIZE), rand_num(BLOCK_SIZE, BLOCK_SIZE * 4 + 1), pixel);
    }

    circles_img->save(circles_img, "img/random_circles.bmp", 8, 8, 8, 0);
    circles_img->free(circles_img);
