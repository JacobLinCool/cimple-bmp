...
static inline void timing_start(const string label) {
    ...
    if (_timing_length == _timing_max_length) {
        ...
        _timing_starts =
            (struct timespec*)realloc(_timing_starts, sizeof(struct timespec) * _timing_max_length);
    }
...
static inline f128 timing_check(const string label) {
    for (u32 i = 0; i < _timing_length; i++) {
        if (strcmp(_timing_labels[i], label) == 0) {
            ...
            return (f128)((end.tv_sec - _timing_starts[i].tv_sec) * 1e3 +
                          (end.tv_nsec - _timing_starts[i].tv_nsec) / 1e6L);
        }
    }
    return (-1.0);
}
 ...
 */
static inline void sleep(u64 ms) {
